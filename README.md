...
![status](https://img.shields.io/badge/phase-2%20complete-0E7C74)
...
- **Plots** — bar (mean + points + error), dot/strip, heatmap (± per-row z-score), box, violin
  (± half), grouped/stacked bar, scatter (linear + LOESS trendline with R²), and line/time-course
  (± SD ribbon, unified hover) — all on a refined, slide-ready theme with the colorblind-safe
  Okabe–Ito palette.
- **Interaction** — hovering a series lifts it to full opacity and dims the rest, with rich
  ...
  Heatmaps get a hover crosshair; XY plots get spike crosshairs.
